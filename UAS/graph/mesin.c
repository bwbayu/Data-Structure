...
void findPath(simpul *source, simpul *dest, int *max, int *total) // prosedur untuk mencari jalur paling panjang dari simpul asal ke simpul tujuan
{
    ...
    else // jika current source node bukan node tujuan
    {
        ...
        while (bantu != NULL)       // menelusuri semua jalur dari node
        {
            ...
            {
                *total = *total + bantu->kontainerJalur;
                findPath(bantu->tujuan, dest, max, total);
                *total = *total - bantu->kontainerJalur;
            }
            bantu = bantu->nextJalur;
        }
    }
    // mengubah tanda variabel visitednya menjadi false
